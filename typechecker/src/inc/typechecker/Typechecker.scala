...
object Typechecker {
  ...
  val EmptyResult: Either[List[TypeError], (Chain[Expr[NameWithType]], Substitution)] = Right((Chain.empty, EmptySubst))
  ...
  def withSimpleType(expr: Expr[Name], typ: Type): Either[List[TypeError], (Expr[NameWithType], Substitution)] =
    withTypeScheme(expr, TypeScheme(typ))

  def withTypeScheme(expr: Expr[Name], typ: TypeScheme): Either[List[TypeError], (Expr[NameWithType], Substitution)] = {
    val exprWithType = expr.map(_.withType(typ))
    Right((exprWithType, EmptySubst))
  }

  def typecheck(expr: Expr[Name], env: Environment, source: String): Either[List[TypeError], (Expr[NameWithType], Substitution)] = expr match {
    ...
    case ref @ Reference(name, pos, _)  =>
      env.get(name).map { typ =>
        trace(s"Reference to $name", pos, typ, source)
        withTypeScheme(ref, typ)
      }.getOrElse(TypeError.singleton(pos, s"Reference to undefined symbol: $name"))

    case If(cond, thenExpr, elseExpr, pos, meta) =>
      for {
        // Typecheck the condition
        (c, s1) <- typecheck(cond, env, source)

        NameWithType(_, condType) = c.meta

        _ = trace("If condition", c.pos, condType, source)

        // Typecheck the then expression
        (t, s2) <- typecheck(thenExpr, env, source)

        NameWithType(_, thenType) = t.meta

        _ = trace("Then expression", t.pos, thenType, source)

        // Typecheck the else expression
        (e, s3) <- typecheck(elseExpr, env, source)

        NameWithType(_, elseType) = e.meta

        _ = trace("Else expression", e.pos, elseType, source)

        (cTp, s4) = condType.instantiate

        // Unify the condition with Boolean
        s5 <- unify(c.pos, cTp, Type.Boolean)
        ...
        s8 <- unify(pos, tTp, eTp)

        s = chainSubstitutions(s1, s2, s3, s4, s5, s6, s7, s8)

        _ = if (s.nonEmpty) scribe.info(NL + "Apply substitution: " + Printer.print(s))

        expr = If(c, t, e, pos, meta.withType(thenType)).substitute(s)

        _ = trace("If expression", expr.pos, expr.meta.typ, source)

      } yield (expr, s)

    case Lambda(params, body, pos, meta) =>
      val typedParams = params.map {
        case Param(name, pos, meta) =>
          Param(name, pos, meta.withSimpleType(TypeVariable()))
      }
      ...
      typedParams.foreach {
        case Param(name, pos, meta) =>
          trace(name, pos, meta.typ, source)
      }
      ...
      for {
        ...
        _ = trace("Lambda body", b.pos, b.meta.typ, source)
        ...
        expr = Lambda(typedParams, b, pos, meta.withSimpleType(tp)).substitute(s)

        _ = trace("Lambda expression", expr.pos, expr.meta.typ, source)

      } yield (expr, s)

    case Apply(fn, args, pos, meta) =>
      val tv = TypeVariable()

      trace("Function application", pos, tv, source)

      for {
        // Typecheck the function
        (f, s1) <- typecheck(fn, env, source)

        _ = trace("Function to apply", f.pos, f.meta.typ, source)
        ...
        (as, s2) <- args.zipWithIndex.foldLeft(EmptyResult) {
          case (resSoFar, (nextArg, idx)) =>
            for {
              ...
              _ = trace(s"Argument ${idx + 1}", typedArg.pos, typedArg.meta.typ, source)
        ...
        _ = trace("Applied type", pos, appliedTp, source)

        (fnTp, s5) = f.meta.typ.instantiate

        // Unify the function type with the actual argument types
        s6 <- unify(pos, fnTp, appliedTp)

        s = chainSubstitutions(s4, s5, s6)

        _ = if (s.nonEmpty) scribe.info(NL + "Apply substitution: " + Printer.print(s))

        expr = Apply(f, argsList, pos, meta.withType(TypeScheme(tv))).substitute(s)

      } yield (expr, s)
  }

  def typecheck(decl: TopLevelDeclaration[Name], env: Environment, source: String): Either[List[TypeError], (TopLevelDeclaration[NameWithType], Environment)] = decl match {
    case Let(name, expr, pos, meta) =>
      typecheck(expr, env, source).flatMap {
        case (checkedExpr, subst) =>
          ...
          trace(name, pos, tp, source)
          Right((Let(name, checkedExpr, pos, meta.withType(tp)), substitute(env.updated(name, tp), subst)))
      }
  }
  ...
  def typecheck(
    ...
    case Module(_, _, _, decls, _, meta) =>

      val initialEnv = importedDecls.mapValues(_.meta.typ)

      val emptyRes: Either[List[TypeError], (Chain[TopLevelDeclaration[NameWithType]], Environment)] =
