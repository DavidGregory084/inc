...
object Resolver {
  ...
  def resolve(
    pattern: Pattern[Pos],
    env: SymbolTable
  ): Resolve[(Pattern[Meta.Untyped], SymbolTable)] = pattern match {
        ...
        .collectFirst { case constrNm @ ConstrName(_, _, _, _) =>
          ...
          patterns
            .foldM((emptyPatterns, env)) {
              case ((patsSoFar, envSoFar), FieldPattern(field, None, pos)) =>
                ...
                if (!members.exists(_.name == fieldName))
                  ...
                  Right(
                    (
                      patsSoFar :+ FieldPattern(
                        field,
                        Option.empty[Pattern[Meta.Untyped]],
                        Meta.Untyped(fieldName, pos)
                      ),
                      envSoFar.withValueName(field, fieldName)
                    )
                  )
              case ((patsSoFar, envSoFar), FieldPattern(field, Some(nextPat), pos)) =>
                ...
                if (!members.exists(_.name == fieldName))
                  ...
                  resolve(nextPat, envSoFar).map { case (resolvedPat, patEnv) =>
                    (
                      patsSoFar :+ FieldPattern(
                        field,
                        Some(resolvedPat),
            ...
            .map { case (resolvedPats, patEnv) =>
              ...
              (
                ConstrPattern(name, alias, resolvedPats.toList, Meta.Untyped(aliasName, pos)),
                aliasEnv
              )
            }
        }
  ...
  def resolve(matchCase: MatchCase[Pos], env: SymbolTable): Resolve[MatchCase[Meta.Untyped]] =
    matchCase match {
  ...
  def resolve(
    expr: TypeConstructorExpr[Pos],
    env: SymbolTable
  ): Resolve[(TypeConstructorExpr[Meta.Untyped], SymbolTable)] =
    expr match {
  ...
  def resolve(
    expr: TypeExpr[Pos],
    env: SymbolTable
  ): Resolve[(TypeExpr[Meta.Untyped], SymbolTable)] = expr match {
    case TypeApplyExpr(typ, args, pos) =>
      ...
      for {
        ...
        (resolvedArgs, _) <- args.foldM(emptyRes) { case ((argsSoFar, updatedEnv), arg) =>
          resolve(arg, updatedEnv).map { case (arg, argEnv) =>
  ...
  def resolve(param: Param[Pos], env: SymbolTable): Resolve[(Param[Meta.Untyped], SymbolTable)] =
    param match {
        ...
        if (env.valueNames.contains(name))
          ...
          ascribedAs match {
            case Some(ascription) =>
              resolve(ascription, env).map { case (resolvedAscription, _) =>
                val localName = LocalName(name)
            ...
            case None =>
              ...
              val paramWithName =
                param.copy(ascribedAs = Option.empty, meta = Meta.Untyped(localName, pos))

              Right((paramWithName, env.withValueName(name, localName)))
          }
    }

  def resolve(expr: Expr[Pos], env: SymbolTable): Resolve[(Expr[Meta.Untyped], SymbolTable)] =
    expr match {
      ...
      case Lambda(params, body, pos) =>
        ...
        val resolvedParams = params.foldM(emptyRes) { case ((paramsSoFar, updatedEnv), param) =>
          resolve(param, updatedEnv).map { case (param, paramEnv) =>
      ...
      case Apply(fn, args, pos) =>
        for {
          ...
          ra <- args.foldM(Chain.empty[Expr[Meta.Untyped]]) { case (argsSoFar, nextArg) =>
            resolve(nextArg, env).map { case (arg, _) =>
              argsSoFar :+ arg
            }
          }
      ...
      case Match(expr, cases, pos) =>
        resolve(expr, env).flatMap { case (m, _) =>
          cases
            .traverse {
              resolve(_, env)
            }
  ...
  def resolve(
    mod: Module[Pos],
    decl: TopLevelDeclaration[Pos],
    env: SymbolTable
  ): Resolve[(TopLevelDeclaration[Meta.Untyped], SymbolTable)] = decl match {
    case Let(name, expr, pos) =>
      val memberName = MemberName(mod.pkg, mod.name, name)
      resolve(expr, env).flatMap { case (resolvedExpr, updatedEnv) =>
      ...
      for {
        (resolvedTparams, tparamEnv) <- tparams.foldM(emptyRes) {
          case ((tparamsSoFar, updatedEnv), tparam @ TypeConstructorExpr(_, _)) =>
            ...
            resolve(tparam, tyVarEnv).map { case (tp, _) =>
              (tparamsSoFar :+ tp, tyVarEnv)
            }
        }

        resolvedCases <- cases.traverse { case constr @ DataConstructor(_, params, constrPos) =>
          ...
          resolvedParams.map { case (parms, _) =>
        ...
        updatedEnv = resolvedCases.foldLeft(env) { case (envSoFar, nextCase) =>
  ...
  def initialPass(
    ...
    decls.foldM(importedEnv) {
      ...
      case (outerEnv, Data(dataNm, _, cases, dataPos)) =>
        ...
        val constrEnv = cases.foldM(dataEnv) { case (env, DataConstructor(name, _, pos)) =>
          if (env.valueNames.contains(name))
            ResolverError.alreadyDefined(pos, name, env.valueNames(name))
          else if (env.typeNames.contains(name))
            ResolverError.alreadyDefined(pos, name, env.typeNames(name))
  ...
  def resolve(
    ...
    case Module(pkg, name, _, decls, pos) =>
      ...
      initialPass(module, decls, importedEnv).flatMap { initialEnv =>
        ...
        decls
          .foldM((emptyDecls, initialEnv)) { case ((resolvedSoFar, envSoFar), nextDecl) =>
            resolve(module, nextDecl, envSoFar).map { case (resolvedDecl, updatedEnv) =>
